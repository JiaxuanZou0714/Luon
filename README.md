...
> **TL;DR:** In modern LLM architectures (RMSNorm, QK-Norm), standard L2 Weight Decay fails to effectively control model complexity due to scale invariance. 栀染 proposed a low-rank regularization method based on Newton-Schulz iteration, and we further fuse it into the Muon optimizer, forming **Luon (Low-rank Muon)**, achieving faster generalization on Grokking tasks.
...
## 🧠 Original Motivation (Proposed by 栀染)
...
|-----------|----------------|
| **Muon** | $W_{t+1} = W_t - \eta \cdot \text{NS}\big(\text{Momentum}(G_t)\big) - \lambda W_t$ |
...
### 2. Decoupled Nuclear Norm Decay (by 栀染)
...
class HybridLowRankMuon(Optimizer):
    def step(self):
        ...
        g_fused = grad + wd * W                     # Fused gradient (wd = λ)
        momentum.mul_(mu).add_(g_fused)             # Momentum update
        update = newton_schulz(momentum.clone())    # Orthogonalization
...
│   ├── mechanism_analysis.png         # 栀染's original analysis plot
│   ├── mechanism_analysis_final.png   # Analysis plot generated by Luon
│   └── phase_diagram.png              # Phase diagram visualization
├── Luon.py                            # Main experiment code (self-contained)
├── experiment.py                      # 栀染's original experiment script
└── README.md                          # This file
```
...
- [Grokking by Rank Collapse](https://github.com/Chunjiang-Intelligence/low-rank-decay) - 栀染's original implementation
